...
The dataset contains order records for six bicycle models, along with a bill of materials describing how each model breaks down into subassemblies, and those subassemblies broken down further. The task is to calculate the total quantity of each individual part requried to fulfill all orders in the dataset.

To solve this problem, I created a recursive CTE. Typically, problems involving tree hierarchies are solved using recursive CTEs. A recursive CTE consists of two parts (anchor & recursive) that are merged per the UNION ALL statement. The first part, the anchor member is the non-recursive part of the query that provides the initial rows in the result set. In this case, this is the query that contains the top level elements of the tree which is the items ordered in the bike_orders table (e.g., Trailhawk 700, Coastal Cruiser, etc.). The recursive member traverses down the hierarchy where the parts have as their parent item the distinct bike models in the bike_orders table. And if the parts are subassemblies, then the recursive member will traverse down the tree again where the individual parts have the subassemblies as parents, and so on.
